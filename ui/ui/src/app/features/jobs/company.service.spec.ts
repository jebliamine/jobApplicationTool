...
describe('CompanyService', () => {
  ...
  it('create() POSTs to /companies', () => {
    service.create({ name: 'Acme', website: null, location: null, notes: null }).subscribe((company) => {
      expect(company).toEqual(COMPANY);
    });

    const req = httpMock.expectOne(`${environment.apiUrl}/companies`);
    expect(req.request.method).toBe('POST');
    expect(req.request.body).toEqual({ name: 'Acme', website: null, location: null, notes: null });
    req.flush(COMPANY);
  });
});
